...
app.post('/api/sync/:username', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log('⚠️ Ошибка получения данных:', err.message);
    }

    // Если не удалось получить данные - используем демо
    if (!profile) {
    ...
    if (posts.length > 0 && !isMockData) {
      for (const post of posts) {
        // Обложка
        let thumbnail = post.display_url || post.thumbnail_src || post.display_src || '';
    ...
    // Если видео нет - генерируем демо
    if (allPosts.length === 0) {
      console.log('🔄 Генерируем демо-рилсы');
      const captions = [
    ...
    console.log(`📹 Сохраняем ${allPosts.length} постов`);
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   POST /api/sync/:username`);
  console.log(`   DELETE /api/user/:username`);
  console.log(`   GET  /api/proxy-image?url=...`);
